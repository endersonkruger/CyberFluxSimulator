...
public class CyberFluxSimulator {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 20; i++) {
            if (i % 3 == 0) {
                clientes[i] = new Gamer("Gamer " + i, gerenciador);  // 1/3 dos clientes são Gamers
            } else if (i % 3 == 1) {
                clientes[i] = new Freelancer("Freelancer " + i, gerenciador);  // 1/3 dos clientes são Freelancers
            } else {
        ...
        for (Cliente cliente : clientes) {
            cliente.start();
            try {
                // Aguarda um tempo aleatório entre a criação de cada thread
...
//Prioridade adaptativa → Se um cliente falhar muitas vezes, ele recebe prioridade maior.
...
//  quanto mais vezes falhou, maior será sua prioridade para alocar recursos futuramente.
//  > também temos um mutex para impedir que outras threads alterem o valor
//  da fila de prioridades fora de ordem
//  > e uma fila de clientes em si (baseado na logica de prioridade), quem estiver
...
//SER VERIFICADO NOVAMENTE! PORQUE, POR EXEMPLO, QUANDO OS CLIENTES NÃO CONSEGUIREM
//ALOCAR ALGUM RECURSO, AS MENSAGENS DISTO PRECISAM SER ENVIADAS NO TERMINAL E ISSO
